...
namespace PathController.Tool
{
    public class SelectInstanceTool : BaseTool
    {
        ...
        {
            RaycastInput nodeInput = new RaycastInput(PathControllerTool.MouseRay, PathControllerTool.MouseRayLength)
            {
            ...
            if (PathControllerTool.RayCast(nodeInput, out RaycastOutput nodeOutput))
            {
        ...
        public override void OnPrimaryMouseClicked(Event e)
        {
            if (HoverValid)
            {
                if (HoveredNodeId != 0)
                {
                    if (PathControllerTool.ShiftIsPressed)
                    {
            ...
            if (HoverValid)
            {
                if (HoveredSegmentId != 0)
                {
                    RenderUtil.RenderRawSegmentOverlay(
                        ...
                        SelectedSegmentIDs.Contains(HoveredSegmentId) ? (PathControllerTool.ShiftIsPressed ? Colors.OrangeWeb : Colors.Add(Colors.GameGreen, 0.15f)) : Colors.GameBlue,
                        0f,
                        0f,
                        true
                    );
                ...
                if (HoveredNodeId != 0)
                {
                    RenderUtil.DrawNodeCircle(
                        cameraInfo,
                        SelectedNodeIDs.Contains(HoveredNodeId) ? (PathControllerTool.ShiftIsPressed ? Colors.OrangeWeb : Colors.Add(Colors.GameGreen, 0.15f)) : Colors.GameBlue,
                        HoveredNodeId,
                        true
                    );
                }
