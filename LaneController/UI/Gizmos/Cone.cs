...
public static class Cone {
    public static GameObject Create(Material material, int subdivisions = 10, float radius =1 , float height =2) {
        var cone = new GameObject("Cone");
        cone.AddComponent<MeshFilter>().sharedMesh = CreateMesh(subdivisions, radius, height);
        cone.AddComponent<MeshRenderer>().sharedMaterial = material;
        return cone;
    }

    static Mesh CreateMesh(int subdivisions, float radius, float height) {
        Mesh mesh = new Mesh();
